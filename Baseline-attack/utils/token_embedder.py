import torch 
import re
from transformers import TextClassificationPipeline, AutoModelForSequenceClassification, AutoTokenizer, LlamaTokenizer, LlamaForCausalLM, AutoModelForCausalLM
import string
...
class Llama3_1Embedding(TokenEmbedding):
    def __init__(self, language_model_vocab_tokens):
        super().__init__(language_model_vocab_tokens)
        # Can be changed to other models
        embedding_model = "meta-llama/Llama-3.1-8B"
        tokenizer = AutoTokenizer.from_pretrained(embedding_model, trust_remote_code=True)
        self.all_vocab = tokenizer.get_vocab()

        lm = AutoModelForCausalLM.from_pretrained(embedding_model, torch_dtype=torch.bfloat16).to(self.torch_device)
        print(lm.get_input_embeddings())
        self.all_embeddings = lm.get_input_embeddings().weight # torch tensor 128256, 4096
        self.set_embeddings() #self.all_embeddings has dim 1, 28158, 4096 after unsqueezing
        self.embed_dim = self.all_embeddings.shape[2]# 4096
        print(self.all_embeddings.shape[2])
...
class Llama2Embedding(TokenEmbedding):
    def __init__(self, language_model_vocab_tokens):
        super().__init__(language_model_vocab_tokens)
        # Can be changed to other models
        ...
        lm = AutoModelForCausalLM.from_pretrained(embedding_model, torch_dtype=torch.bfloat16).to(self.torch_device)
        print(lm.get_input_embeddings())
        self.all_embeddings = lm.get_input_embeddings().weight 
        self.set_embeddings() 
        self.embed_dim = self.all_embeddings.shape[2]
        print(self.all_embeddings.shape[2])
...
class VicunaEmbedding(TokenEmbedding):
    def __init__(self, language_model_vocab_tokens):
        super().__init__(language_model_vocab_tokens)
        # Can be changed to other models
        embedding_model = "lmsys/vicuna-7b-v1.5"
        tokenizer = AutoTokenizer.from_pretrained(embedding_model, trust_remote_code=True)
        self.all_vocab = tokenizer.get_vocab()
...
class MistralEmbedding(TokenEmbedding):
    def __init__(self, language_model_vocab_tokens):
        super().__init__(language_model_vocab_tokens)
        # Can be changed to other models
        embedding_model = "mistralai/Mistral-7B-Instruct-v0.3"
        tokenizer = AutoTokenizer.from_pretrained(embedding_model, trust_remote_code=True)
        self.all_vocab = tokenizer.get_vocab()
