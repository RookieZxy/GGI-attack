...
# def token_gradients(model, input_ids_list, input_slice_list, target_slice_list, loss_slice_list, device, labels):
#     embed_weights = get_embedding_matrix(model)
#     loss_list = []
#     embeds_nonQuery = None
#     last_stop = None
...
#             for index, input_slice in enumerate(input_slice_list):
#                 if last_stop == None:
#                     embeds[:,:input_slice.start,:]
#                     embeds_nonQuery = torch.cat(
#                     [
#                         embeds[:,:input_slice.start,:], 
#                         input_embeds[:, index:index+1, :], 
#                     ], 
#                     dim=1)
#                 else:
#                     embeds_nonQuery = torch.cat(
#                     [
#                         embeds_nonQuery,
#                         embeds[:,last_stop:input_slice.start,:],
#                         input_embeds[:, index:index+1, :], 
#                     ], 
#                     dim=1)
#                 last_stop = input_slice.stop
...
#         logits = model(inputs_embeds=full_embeds).logits
#         targets = input_ids[target_slice_list[index1]]
#         # print(logits[0,loss_slice_list[index1],:].shape)
#         # print(targets)

#         next_token_logits = logits[0,loss_slice_list[index1],:]
#         other = next_token_logits[0][0]
#         for index in range(next_token_logits.shape[1]):
#             if index != 31591 and index != 24561:
#                 other += next_token_logits[0][index]
#         positive = next_token_logits[0][24561]
#         negative = next_token_logits[0][31591]
#         # other = torch.tensor([other])

#         # preds = torch.tensor([positive, negative, other])
#         preds = torch.stack([positive, negative, other]).to(device)
#         # preds.retain_grad
